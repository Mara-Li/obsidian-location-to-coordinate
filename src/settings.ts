...
class SettingTab extends PluginSettingTab {
	...
	async display(): Promise<void> {
		...
		new Setting(containerEl).setNames("inputKeys.header").setHeading();
		...
		await MarkdownRenderer.render(
			this.app,
			dedent(`${i18next.t("inputKeys.desc")}`),
			inputSettings.descEl,
			"",
			component
		);
		component.onunload();
		...
		if (this.plugin.settings.inputKeys.mode === "simple") {
			new Setting(containerEl).setNames("keyName").addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.inputKeys.object).onChange(async (value) => {
		 ...
		 * Output Format
		 */
		new Setting(containerEl).setNames("outputFormat.header").setHeading();
		...
		await MarkdownRenderer.render(
			this.app,
			dedent(`${i18next.t("outputFormat.desc")}`),
			outputSettings.descEl,
			"",
			component2
		);
		component2.onunload();
		...
		if (this.plugin.settings.outputFormat.mode === "simple") {
			new Setting(containerEl).setNames("latitude.name").addText((text) =>
				text
			...
			new Setting(containerEl).setNames("longitude.name").addText((text) =>
				text
			...
			new Setting(containerEl)
				.setNames("template")
				.setDesc(
					sanitizeHTMLToDom(`${i18next.t("outputFormat.template", { code: codeKeys })}`)
				)
				.addTextArea((text) =>
					text
